...
HWND * CTaskPage::GetHWNDS(BOOL fSelectedOnly, DWORD * pdwCount)
{
    ...
    if (fSelectedOnly)
    {
        ...
        pArray = (CPtrArray *)GetSelectedTasks();
        if (NULL == pArray)
        {
            return NULL;
        }
    ...
    else
    {
        ...
        if (pArray)
        {
            if (FALSE == pArray->Copy(*(CPtrArray *)m_pTaskArray))
            {
                delete pArray;
                *pdwCount = 0;
                return NULL;
...
// Actually CPtrArray
void * CTaskPage::GetSelectedTasks()
{
    BOOL fSuccess = TRUE;
...
void CTaskPage::HandleTaskListContextMenu(INT xPos, INT yPos)
{
    ...
    CPtrArray *pArray = (CPtrArray *)GetSelectedTasks();
    ...
    case LVN_COLUMNCLICK:
        {
            ...
            ResortTaskArray((CPtrArray **)&m_pTaskArray);
            TimerEvent();
        }
        break;
...
VOID CTaskPage::TimerEvent()
{
    ...
    te.m_pTasks = (CPtrArray *)m_pTaskArray;
    te.m_pPage  = this;
    te.lpWinsta = NULL;
    te.lpDesk   = NULL;
    te.uPassCount = uPassCount;
    ...
    if ( DoEnumWindowStations( EnumWindowStationsFunc, (LPARAM) &te ))
    {
        ...
        while (i < m_pTaskArray->GetSize())
        {
            ...
            if (pTaskInfo->m_uPassCount != uPassCount)
            {
...
BOOL CALLBACK EnumWindowsProc(HWND    hwnd, LPARAM   lParam)
{
    ...
    for (i=0; i < numTasks; i++) 
    {
        ...
        if (pTask->m_hwnd == hwnd)
        {
            ...
            pTask->m_uPassCount = te->uPassCount;
...
HRESULT CTaskPage::Initialize(HWND hwndParent)
{
    ...
    *(CPtrArray **)&m_pTaskArray = new CPtrArray(GetProcessHeap());
